...
export default function Home() {
  ...
  const renderDay = (day, index) => {
    ...
    return (
      <div key={index} className={`mb-4 rounded-lg shadow-md overflow-hidden
        ${isCurrentDay ? 'ring-2 ring-red-300 bg-red-50/50' : ''}
        ...
        <div className="p-4">
          <div className="flex justify-between items-center">
            <h2 className={`text-xl text-gray-900 ${isCurrentDay ? 'font-bold' : 'font-semibold'}`}>
              ...
              {isCurrentDay && <span className="ml-2 text-xs font-medium text-red-600 bg-red-100 px-2 py-0.5 rounded-full align-middle">Heute</span>}
            </h2>
  ...
  return (
    <>
      ...
      {/* Neujahrs-Animation mit anime.js - 3 Phasen */}
      {showNewYear && (
        <>
          <style jsx>{`
            .newyear-anime-container {
              position: fixed;
              top: 0;
              left: 0;
              width: 100vw;
          ...
          <div className="newyear-anime-container">
            {/* Phase 1: Schweinchen-Rakete fliegt */}
            {newYearPhase === 1 && (
              <img 
                ref={newYearRocketRef}
                src="/newyear-rocket.png" 
                alt="Schweinchen auf Rakete"
                className="newyear-rocket"
              />
            )}
            
            {/* Canvas für Feuerwerk und Konfetti */}
            <canvas 
              ref={newYearCanvasRef}
              className="newyear-canvas"
            />
          </div>
        </>
      )}
