  ...
  return (
    <>
      ...
      <header className={`${isKioskMode ? 'mb-4' : 'mb-8'}`}>
        <h1 className="text-3xl font-bold text-gray-900">Betriebskantine</h1>
        <div className={`${isKioskMode ? 'mt-2 p-3' : 'mt-4 p-4'} bg-white rounded-lg shadow-sm border border-gray-100`}>
          <div className={`${isKioskMode ? 'flex justify-center' : 'grid grid-cols-2 gap-4'}`}>
            {/* Linke Spalte: Öffnungszeiten */}
            <div>
              <h2 className={`${isKioskMode ? 'text-base' : 'text-lg'} font-semibold text-gray-800 ${isKioskMode ? 'mb-1' : 'mb-2'}`}>Öffnungszeiten</h2>
              <div className={`flex ${isKioskMode ? 'space-x-6 text-sm' : 'space-x-8'}`}>
                <div>
            ...
            {/* Rechte Spalte: Kontaktinfo - nur im normalen Modus */}
            {!isKioskMode && (
            ...
            {/* Postfremde-Info im Kiosk-Modus */}
            {isKioskMode && (
              <p className="text-gray-600 text-sm mt-2 text-center">Für Postfremde erhöht sich der Preis um {menu.contactInfo?.postcode} €</p>
            )}
          </div>
        </div>
      </header>
