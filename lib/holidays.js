...
const LEGAL_HOLIDAYS = {
  '01-01': { name: 'Neujahr', type: 'holiday', isLegalHolidayInLowerSaxony: true },
  '05-01': { name: 'Tag der Arbeit', type: 'holiday', isLegalHolidayInLowerSaxony: true },
  '10-03': { name: 'Tag der Deutschen Einheit', type: 'holiday', isLegalHolidayInLowerSaxony: true },
  '12-25': { name: 'Weihnachten', type: 'holiday', isLegalHolidayInLowerSaxony: true },
  '12-26': { name: '2. Weihnachtstag', type: 'holiday', isLegalHolidayInLowerSaxony: true }
};

// Bewegliche gesetzliche Feiertage in Niedersachsen:
// - Karfreitag
// - Ostermontag
// - Christi Himmelfahrt
// - Pfingstmontag
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (currentDate.getDate() === weiberfastnacht.getDate() && 
      ...
      result[dateStr] = {
        ...
        color: 'pink',
        forceClose: false,
        isLegalHolidayInLowerSaxony: false
      };
      continue;
    }
    ...
    if (holiday) {
      // Prüfe ob es ein gesetzlicher Feiertag in Niedersachsen ist
      const isLegal = holiday[0].type === 'public';
      result[dateStr] = {
        name: holiday[0].name,
        type: 'holiday',
        color: 'red',
        forceClose: isLegal,
        isLegalHolidayInLowerSaxony: isLegal
      };
    }
  }
...
export function checkDay(dateString) {
  ...
  if (LEGAL_HOLIDAYS[mmdd]) {
    return { ...LEGAL_HOLIDAYS[mmdd], forceClose: true };
  }
